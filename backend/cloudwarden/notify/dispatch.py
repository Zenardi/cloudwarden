...
def dispatch_for_anomaly(
    ...
    *,
    ...
    """Render ``template_id`` against ``context`` and send it via the named channel (M14.3).

    Anomaly alerts are system-wide, so the destination is a **channel resolved by name**
    (``settings.anomaly_alert_channel``) rather than a per-object id. Returns ``None``
    when no channel name is configured or no such channel exists — detection then
    records the anomaly silently. Reuses the same transport registry + :func:`service.notify`
    seam as bindings/budgets, so no new delivery code path is added."""
    if not channel_name:
        return None
...
def dispatch_for_drift(
    ...
    *,
    ...
    """Render ``template_id`` against ``context`` and send it via the named channel (M14.7).

    Drift alerts are system-wide, so the destination is a **channel resolved by name**
    (``settings.drift_alert_channel``) rather than a per-object id. Returns ``None`` when no
    channel name is configured or no such channel exists — detection then records the drift
    silently. Reuses the same transport registry + :func:`service.notify` seam as the other
    alerts, so no new delivery code path is added."""
    if not channel_name:
        return None
    channel = repo.get_notification_channel_by_name(session, channel_name)
    if channel is None:
