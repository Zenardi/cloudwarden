...
def _parse_response(payload: dict[str, Any], cost_type: str) -> list[CostRow]:
...
def _collect_live(
    ...
    with httpx.Client(timeout=60.0) as http:
        for cost_type in ("Amortized", "Actual"):
            ...
            while next_url:
                resp = http.post(next_url, headers=headers, json=body)
                resp.raise_for_status()
                payload = resp.json()
                out.extend(_parse_response(payload, cost_type))
